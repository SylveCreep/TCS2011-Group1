...
public interface UserDao extends JpaRepository<User, Long> {
    ...
    @Query(value = "Select * from user u " + "LEFT JOIN faculty f ON f.id = u.faculty_id "
            ...
            + "AND ((:gender is null) or (u.gender = :gender)) " + "AND ((:userId is null) or (u.id = :userId)) "
            + "AND ((:roleId is null) or (r.id = :roleId)) " + "AND ((:facultyId is null) or (f.id = :facultyId)) "
            + "group by u.id", nativeQuery = true)
    Page<User> searchUserByRoleAndFac(@Param("userId") Long userId, @Param("roleId") Long roleId,
            @Param("facultyId") Long facultyId, @Param("fullName") String fullName, @Param("roleName") String roleName,
            @Param("facultyName") String facultyName, @Param("email") String email, @Param("startDate") Date startDate,
            @Param("endDate") Date endDate, @Param("hasDate") int hasDate, @Param("gender") Integer gender,
            @Param("code") String code, Pageable pageable);

    @Query(value = "Select * FROM user u " + "LEFT JOIN faculty f ON f.id = u.faculty_id " + "WHERE u.is_deleted = 0 "
