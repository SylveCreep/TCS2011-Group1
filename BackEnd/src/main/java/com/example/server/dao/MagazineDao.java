...
public interface MagazineDao extends JpaRepository<Magazine, Long> {
        ...
        @Query(value = "SELECT COUNT(c.id) as totalContribution, m.theme as magazineTheme from magazine m "
                        + "LEFT JOIN contribution c ON c.magazine_id = m.id " + "WHERE c.is_deleted = 0 "
                        + "and m.is_deleted = 0 " + "group by m.theme, m.id ", nativeQuery = true)
        List<Object> countContributionsByMagazine();
}
